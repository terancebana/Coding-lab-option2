...
echo "Select log file to analyze:" [cite: 6]
echo "1) Heart Rate (heart_rate_log.log)" [cite: 6]
echo "2) Temperature (temperature_log.log)" [cite: 6]
echo "3) Water Usage (water_usage_log.log)" [cite: 6]
...
case $choice in
    ...
    3)
        ...
        ;;
    *)
        handle_error "Invalid user input. Please choose 1, 2, or 3." [cite: 6]
        ;;
esac
...
awk '{print $2}' "$LOG_PATH" | sort | uniq -c | while read -r count device; do
    echo "  $device: $count" >> "$ANALYSIS_REPORT_FILE"
done
